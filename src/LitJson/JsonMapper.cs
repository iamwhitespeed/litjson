...
namespace LitJson
{
    ...
    public class JsonMapper
    {
        ...
        static BindingFlags pflags = ~BindingFlags.InvokeMethod & (BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic);
        private static BindingFlags fflags = pflags;
        static JsonMapper()
        {
            maxNestingDepth = 100;
        ...
        private static ObjectMetadata AddObjectMetadata(Type type)
        {
            ...
            {
                if (typeof(Delegate).IsAssignableFrom(pinfo.PropertyType))
                    continue;
                if (pinfo.Name == "Item")
                {
                    ParameterInfo[] parameters = pinfo.GetIndexParameters();
            ...
            {
                if(typeof(Delegate).IsAssignableFrom(finfo.FieldType))
                    continue;;   
        ...
        {
            reader.Read();
            if (reader.Token == JsonToken.ArrayEnd || reader.Token == JsonToken.None)
            {
                return null;
            }
            ...
            else if (reader.Token == JsonToken.ObjectStart)
            {
                ...
                // Once the object is Deserializing, it will be invoked to create the actual converted object.
                ...
                if (valueType == typeof(JsonData))
                    ((JsonData)instance).SetJsonType(JsonType.Object);
                bool firstRun = true;
                while (!done)
                {
                    ...
                    {
                        // Don't Deserializing a field or property that has a JsonIgnore attribute with Deserializing usage.
        ...
        private static void RegisterBaseImporters()
        {
            ...
            RegisterImporter(baseImportTable, typeof(string), typeof(float), delegate (object input)
            {
                return float.Parse((string)input);
            });
            RegisterImporter(baseImportTable, typeof(string), typeof(bool), delegate (object input)
            {
                return bool.Parse((string)input);
            });
        }
        ...
        {
            JsonWriter writer = new JsonWriter() { PrettyPrint = pretty };
